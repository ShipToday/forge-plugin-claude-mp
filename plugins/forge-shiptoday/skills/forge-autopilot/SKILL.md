...
- Read-only inspection: `Read`, `Grep`, `Glob`, `TodoWrite`, plus read-only
  MCP tools (names starting with `list_`,
  `get_`, `search_`, `query_`, `fetch_`, `notion-search`, `notion-fetch`)
- Recovery and coordination: `ToolSearch`, `Skill`, `ScheduleWakeup`,
  `TaskOutput`, `ListAgents`, `Agent`, `Task`, and the host task
  tools `TaskCreate`, `TaskUpdate`, `TaskList`, `TaskGet`. A bounded shell
  inspection command may also be allowed by the host hook; it is not general
  shell access.
...
- **On first sight, render it even if you think a hook already did.** Some
  clients ship Forge's `must-display` PostToolUse hook, but hook output is not
  proof that the user saw it. The only safe suppression is an exact event id
  replay that you already rendered in one of your own user-facing messages.
...
to execute non-trivial work (e.g., Claude Code auto-enters plan mode on
verbs like "implement", "fix", "build"). In plan mode, the harness forbids
...
3. **Defer writes** — surface every write the workflow would normally
   ...
   `ExitPlanMode`. Do NOT execute those writes while plan mode is active.
