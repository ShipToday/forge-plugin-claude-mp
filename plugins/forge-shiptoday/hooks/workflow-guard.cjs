...
const ALWAYS_ALLOWED_BARE_NAMES = new Set([
  ...
  'Glob',
  'TodoWrite',
  // Deferred-tool discovery and skill loading are required before a fresh
  // Claude Code process can call Forge at a gate.
  'ToolSearch',
  'Skill',
  // Waiting and coordination do not advance the pinned workflow. Claude Code
  // applies the parent's session pin to child tool calls, so a delegated task
  // remains subject to this guard when it attempts a gated write.
  'ScheduleWakeup',
  'TaskOutput',
  'ListAgents',
  'Agent',
  'Task',
  // Current host task/todo names. These are local task coordination, not a
  // blanket permission for connector or filesystem writes.
  'TaskCreate',
  'TaskUpdate',
  'TaskList',
  'TaskGet',
...
function buildCheckpointDenyReason(state, toolName) {
  ...
  const lines = [
    ...
    '  1. Relay the pending question with the user-question tool available on this host (AskUserQuestion on Claude Code; request_user_input on Codex), or present numbered choices in your reply when no native question tool is callable.',
...
function deny(reason) {
  // Claude Code validates PreToolUse denials only in this host-specific shape.
...
function buildStepPermissionDenyReason(state, toolName, category) {
  ...
  const lines = [
    ...
    '  2. If the user must decide before this step can complete, use the host user-question tool (AskUserQuestion on Claude Code; request_user_input on Codex), or present numbered choices in your reply when no native question tool is callable.',
    '  3. Call forge__abandon_workflow with a meaningful reason ONLY if the workflow itself no longer applies (wrong workflow, user redirected).',
    '     Never abandon to skip the remaining steps: a post-step confirmation gate already offers the user "Stop here" for that — relay it.',
    ``,
    'Do NOT silently bypass the workflow. The audit trail is how the team learns when workflows misroute.',
